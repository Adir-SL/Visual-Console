...
function addConsole(){
    ...
    ConsoleStyles("#vconsole_"+window.checkTime+"{ position:absolute; right:16px; top:16px; width:300px; max-width:50%; height:150px; max-height:25%; padding:8px; overflow:auto; scroll-behavior:smooth; background-color:#ffffff; box-shadow:0 4px 16px 0 rgba(0,0,0,.4); font-family:sans-serif; border-radius: 8px;transition:height 200ms ease-in-out, opacity 200ms ease-in-out; }"+"#vconsole_"+window.checkTime+" div{ margin-bottom:4px; padding:8px; border-radius:8px; background-color:#ededed; }"+"#vconsole_"+window.checkTime+" summary:empty ~ div:last-of-type{ animation:slideOver 100ms ease-out; }"+"#vbutton_"+window.checkTime+"{ position:fixed; right:32px; top:32px; padding:1px; width:32px; height:32px; border:none; transform:translate(80%,-80%); transition:transform 200ms cubic-bezier(.62,-0.71,.43,1.57) 100ms; outline:none; background-color:skyblue; color:#ffffff; font-weight:bold; border-radius:50%; display: flex; align-items: center; justify-content: center; cursor:pointer; box-shadow:0 4px 8px 0 rgba(0,0,0,.4); } @keyframes slideOver{ 0%{transform: translateX(-100%);} 100%{transform: translateX(0px);}}"+"#vconsole_"+window.checkTime+" + .flexDiv{ display:flex; align-items:stretch; position:absolute; top:180px; right:16px; width: 300px; padding: 8px; margin-top: 8px; border-radius:8px; box-shadow:0 4px 16px 0 rgba(0,0,0,.4); background-color:#ffffff;transition:opacity 200ms ease-in-out; }"+"#vconsole_"+window.checkTime+" + .flexDiv input{ width:calc(100% - 64px); background-color:#ffffff; border-radius:8px; padding: 8px 16px; border:1px solid #cccccc; outline:none;margin-right:8px; font-size: 16px; -webkit-appearance: none; -moz-appearance: none; appearance: none; }"+"#ibutton_"+window.checkTime+"{ border-radius:8px; border:none; outline:none; background-color:skyblue; color:#ffffff; padding:1px; width:36px; font-weight:bold; display: flex; align-items: center; justify-content: center; cursor:pointer; }"+"#vbutton_"+window.checkTime+" svg{ transition:transform 200ms ease-in-out; }"+"#ibutton_"+window.checkTime+" svg{ transform:rotate(-90deg); }")
...
function toggleConsole(){
    if(window.hideConsole != true){
        ...
        // document.getElementById('vbutton_'+window.checkTime).style.transform = "translate(50%,-50%)";
        ...
        // document.getElementById('vbutton_'+window.checkTime).style.transform = "translate(0,0)";
